---
name: impeccable
description: "Use when the user wants to JUDGE or IMPROVE a frontend interface — critique, audit, review, redesign, polish, distill, harden, simplify, or make a design bolder, quieter, or more delightful. Owns design judgement: what is wrong with this interface, and what would make it better. Covers UX review, visual hierarchy, information architecture, cognitive load, responsive behaviour, anti-patterns, spacing, alignment, motion, micro-interactions, UX copy, error and empty states, and live browser iteration. Do NOT use it to WRITE component code with shadcn, Radix or Tailwind (use ui-styling), to LOOK UP a palette, font pairing, icon or UX guideline (use ui-ux-pro-max), to build a token architecture (use design-system), for banners (use banner-design), or for other brand assets such as logos, slides and social images (use design). Not for backend-only or non-UI tasks."
version: 4.1.2
user-invocable: true
