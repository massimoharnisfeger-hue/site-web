---
name: ui-ux-pro-max
description: "Searchable REFERENCE DATA for interface decisions, to look up a concrete value or precedent rather than to judge or build: 79 styles (50 active), 192 product palettes with reasoning profiles, 74 font pairings, 119 UX guidelines, 105 icons, 17 GSAP presets, 25 chart types, 22 technology stacks. Use when the question is which palette, which font pairing, which icon, which guideline, which animation preset, or what a given stack expects. Do NOT use it as the main skill to critique or redesign an interface (use impeccable) or to write component code (use ui-styling)."
---
